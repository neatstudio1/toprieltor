...
### 7.1. Правка контента в Strapi без изменения кода

Если поменялся только текст в CMS, пересобирать сайт не нужно — но и само оно появится не сразу.

Страницы стоят на ISR (`export const revalidate = 3600`), а `strapiGet` дополнительно кэширует сами
ответы Strapi (`next: { revalidate }`). Два кэша вложены друг в друга: регенерация страницы берёт
данные из fetch-кэша, а тот отдаёт устаревший ответ и обновляется только фоном. На практике это
значит, что правка в Strapi может не показаться и через два часа: первая регенерация отрисует
страницу по старым данным и снова пометит её свежей на час.

Чтобы выкатить контент сразу:

```bash
cd /var/www/ysrealty/apps/web
pm2 stop web
rm -rf .next/cache/fetch-cache
pm2 start web
```

Чистится только кэш HTTP-ответов Strapi — сборка (`.next/server`, `.next/static`) не трогается,
поэтому пересборка не нужна. После рестарта первые запросы к каждой странице идут в Strapi напрямую;
на текущем объёме (около 5700 записей в кэше) он это выдерживает, отвечая за единицы миллисекунд.

Проверять надо именно отрендеренную страницу, а не API: `curl` по адресу статьи и grep по новому
куску текста. Ответ `200` от Strapi ничего не говорит о том, что увидит посетитель.
